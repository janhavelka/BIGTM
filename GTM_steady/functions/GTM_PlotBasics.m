...
function handles=GTM_PlotBasics(MESH,PROBLEM,LOAD,RESP,MDATA,PlotID,handles)
...
if PlotID == 3
    if isempty(handles)
        ...
        % draw the original potential field
%         pot_field_true = trimesh(MESH.Elements(:,1:3),MESH.Nodes(:,1),MESH.Nodes(:,2),LOAD.M_f(:,1));
        pot_field_true = trimesh(MDATA.MESH.Elements(:,1:3),MDATA.MESH.Nodes(:,1),MDATA.MESH.Nodes(:,2),MDATA.LOAD.M_f(:,1));
...
%         e_nodes=MESH.Boundary.R_s(2:3,LOAD.Electrodes.e_idx(:,i))';
%         % coords for electrodes
%         X(:,i)=[MESH.Nodes(e_nodes(:,1),1),MESH.Nodes(e_nodes(:,2),1)]';
%         Y(:,i)=[MESH.Nodes(e_nodes(:,1),2),MESH.Nodes(e_nodes(:,2),2)]';
%         Z(:,i)=[LOAD.M_cf(LOAD.Electrodes.Elements(:,1),1),LOAD.M_cf(LOAD.Electrodes.Elements(:,2),1)]';
% end
...
%         Z=[LOAD.M_f(LOAD.Electrodes.Elements(:,1),1),LOAD.M_f(LOAD.Electrodes.Elements(:,2),1)]';
%         electrode_scheme_true=plot3(X,Y,Z,'b','LineWidth',3);
...
%         plot(X,Y,'r','LineWidth',3);
        ...
        sigma_field=BasePlot( MESH,MESH.ETN*RESP.sigma,[1,3,3],{'$x\,\mathrm{[m]}$','$y\,\mathrm{[m]}$','$\lambda\quad\mathrm{[\frac{W}{m^2\cdot ^oC}]}$','$\mathrm{Conductivity\,field}$'},mode );
        
%         sigma_true_field = trimesh(MESH.Elements(:,1:3),MESH.Nodes(:,1),MESH.Nodes(:,2),MESH.ETN*RESP.sigma_true);
        sigma_true_field = trimesh(MDATA.MESH.Elements(:,1:3),MDATA.MESH.Nodes(:,1),MDATA.MESH.Nodes(:,2),MDATA.MESH.ETN*MDATA.MATERIAL.sigma_true);
        ...
        min_zval=min([MDATA.MATERIAL.sigma_true(:);RESP.sigma(:)]);
        max_zval=max([MDATA.MATERIAL.sigma_true(:);RESP.sigma(:)]);
...
%             mat.Quality=PROBLEM.Video.Quality;
        ...
        for c_idx=1:size(LOAD.M_cf,2)
            ...
            UpdatePlot( pot_field_true,MDATA.LOAD.M_f(:,c_idx) );
%             UpdatePlot( electrode_scheme,[LOAD.M_cf(LOAD.Electrodes.Elements(:,1),c_idx),LOAD.M_cf(LOAD.Electrodes.Elements(:,2),c_idx)]);
%             UpdatePlot( electrode_scheme_true,[LOAD.M_f(LOAD.Electrodes.Elements(:,1),c_idx),LOAD.M_f(LOAD.Electrodes.Elements(:,2),c_idx)]);
        ...
        % VIDEO RECORD
electrode_scheme=[];
electrode_scheme_true=[];
        handles={pot_field,electrode_scheme,grad_field,sigma_field,pot_field_true,electrode_scheme_true};
    else % if you have the handles - just update the fields
        % Gradient of potential field
        if isvalid( handles{3} )
        ...
        for c_idx=1:size(LOAD.M_cf,2)
            ...
            UpdatePlot( handles{5},MDATA.LOAD.M_f(:,c_idx) );
%             UpdatePlot( handles{2},[LOAD.M_cf(LOAD.Electrodes.Elements(:,1),c_idx),LOAD.M_cf(LOAD.Electrodes.Elements(:,2),c_idx)]);
%             UpdatePlot( handles{6},[LOAD.M_f(LOAD.Electrodes.Elements(:,1),c_idx),LOAD.M_f(LOAD.Electrodes.Elements(:,2),c_idx)]);
            if isvalid( handles{3} )
...
if PlotID == 4
    if isempty(handles)
        ...
        % plot solution field (material in each iteration)
        sigma_field=BasePlot( MESH,MESH.ETN*RESP.sigma,[1,3,3],{'$x\,\mathrm{[m]}$','$y\,\mathrm{[m]}$','$\lambda\quad\mathrm{[W/(m\cdot K)]}$','$\mathrm{Conductivity\,field}$'},mode );
...
%             pot.Quality=PROBLEM.Video.Quality;
            ...
            err = VideoWriter('err.avi','Uncompressed AVI');
%             grad = VideoWriter('grad.avi','Motion JPEG AVI');
%             grad.Quality=PROBLEM.Video.Quality;
...
%         end
        ...
        for c_idx=1:size(LOAD.M_cf,2)
            % update the current field
            UpdatePlot( pot_field,LOAD.M_cf(:,c_idx) );
            UpdatePlot( pot_field_true,LOAD.M_f(:,c_idx) );
            UpdatePlot( electrode_scheme,[LOAD.M_cf(LOAD.Electrodes.Elements(:,1),c_idx),LOAD.M_cf(LOAD.Electrodes.Elements(:,2),c_idx)]);
            UpdatePlot( electrode_scheme_true,[LOAD.M_f(LOAD.Electrodes.Elements(:,1),c_idx),LOAD.M_f(LOAD.Electrodes.Elements(:,2),c_idx)]);
            % VIDEO RECORD
            if PROBLEM.Video.Record
        ...
        % VIDEO RECORD
        handles={pot_field,electrode_scheme,err_field,sigma_field,pot_field_true,electrode_scheme_true};
    else % if you have the handles - just update the fields
        ...
        for c_idx=1:size(LOAD.M_cf,2)
            % update the current field
            UpdatePlot( handles{1},LOAD.M_cf(:,c_idx) );
            UpdatePlot( handles{5},LOAD.M_f(:,c_idx) );
            UpdatePlot( handles{2},[LOAD.M_cf(LOAD.Electrodes.Elements(:,1),c_idx),LOAD.M_cf(LOAD.Electrodes.Elements(:,2),c_idx)]);
            UpdatePlot( handles{6},[LOAD.M_f(LOAD.Electrodes.Elements(:,1),c_idx),LOAD.M_f(LOAD.Electrodes.Elements(:,2),c_idx)]);
            pause(video_pause);
            % VIDEO RECORD
...
if PlotID == 5
    ...
    % plot solution field (material in each iteration)
    sigma_field=BasePlot( MESH,MESH.ETN*RESP.sigma,[1,2,2],{'$x\,\mathrm{[m]}$','$y\,\mathrm{[m]}$','$\lambda\quad\mathrm{[S/m]}$','$\mathrm{Conductivity\,field}$'},mode );
    sigma_true_field = trimesh(MESH.Elements(:,1:3),MESH.Nodes(:,1),MESH.Nodes(:,2),MESH.ETN*RESP.sigma_true);
    set(sigma_true_field,'EdgeColor','black','EdgeAlpha',0.6,'FaceColor',[0.9 0.9 0.9],'FaceAlpha',0.5,...
        'AmbientStrength',0.7,...
        'DiffuseStrength',0.8,...
...
if PlotID == -2
    ...
    %     if strncmpi(mode,'normal',4)
    % plot the identifiers
    t_e = text(mean(X),mean(Y),num2str((1:LOAD.Electrodes.nelements)'),'Color','blue','HorizontalAlignment','center','VerticalAlignment','bottom');
    ...
    if strncmpi(mode,'normal',4)
        ...
        scatter(MESH.Nodes(any(LOAD.Electrodes.m_nodes,2),1),MESH.Nodes(any(LOAD.Electrodes.m_nodes,2),2),25,'MarkerEdgeColor',[.7 .1 .1],...
    ...
    if strncmpi(mode,'present',4)
        % plot the measurement nodes
        % any - for ALL inject patterns
        scatter(MESH.Nodes(any(LOAD.Electrodes.m_nodes,2),1),MESH.Nodes(any(LOAD.Electrodes.m_nodes,2),2),210,'MarkerEdgeColor',[.7 .1 .1],...
            'MarkerFaceColor',[0.7 .1 .1],...
            'LineWidth',1.5);
        for i = 1:length(t_e)
            t_e(i).FontSize = 65;
...
if PlotID == -3
    ...
    for i=1:LOAD.Electrodes.nelements
        ...
        e_nodes=MESH.Boundary.R_s(2:3,LOAD.Electrodes.e_idx(:,i))';
        % coords for electrodes
        X=[MESH.Nodes(e_nodes(:,1),1),MESH.Nodes(e_nodes(:,2),1)]';
        Y=[MESH.Nodes(e_nodes(:,1),2),MESH.Nodes(e_nodes(:,2),2)]';
        % plot the electrodes
        if strncmpi(mode,'present',4)
            plot(X,Y,'b','LineWidth',8);
            text(mean(X(:)),mean(Y(:)),num2str(i),'Color','blue','HorizontalAlignment','center','VerticalAlignment','bottom','FontSize',65);
        else
            plot(X,Y,'b','LineWidth',2);
            title('Electrode scheme')
            text(mean(X(:)),mean(Y(:)),num2str(i),'Color','blue','HorizontalAlignment','center','VerticalAlignment','bottom');
        end
        % plot the identifiers
