...
%     LOAD.K_e{i}=sparse(MESH.Boundary.R_s(:,e_idx),MESH.Boundary.C_s(:,e_idx),MESH.Boundary.G_c(:,e_idx)/PROBLEM.Electrode.Im,MESH.nnodes,MESH.nnodes);
    LOAD.K_e{i}=sparse(MESH.Boundary.R_s(:,LOAD.Electrodes.e_idx(:,i)),MESH.Boundary.C_s(:,LOAD.Electrodes.e_idx(:,i)),MESH.Boundary.G_c(:,LOAD.Electrodes.e_idx(:,i))/PROBLEM.Electrode.Im,MESH.nnodes,MESH.nnodes);
end
...
for i=1:LOAD.Electrodes.nelements
    % find particular electrode element on a boundary and conflict with boundary conditions
    e_elements=MESH.Boundary.R_s(2:3,LOAD.Electrodes.e_idx(:,i))';
    e_idx = ~any([ismember(c(1:2,:)',e_elements,'rows'),ismember(c(1:2,:)',fliplr(e_elements),'rows')],2);
    r_te=r(:,e_idx);
    c_te=c(:,e_idx);
    v_te=v(:,e_idx);
