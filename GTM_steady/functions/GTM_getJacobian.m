function J = GTM_getJacobian(MESH,LOAD,MATERIAL)
...
ref_resp=zeros(MESH.nnodes,LOAD.Electrodes.nelements);
K_u=sparse(MESH.R_s,MESH.C_s,MESH.B_c.*MATERIAL.sigma(:,ones(1,9))');
for i=1:LOAD.Electrodes.nelements
    ref_resp(:,i)=(K_u+LOAD.K_t{i}+LOAD.K_e{i})\(LOAD.RHS_e{i}+LOAD.RHS_t{i});
end
% separate the measurements for electrode voltages and the rest nodal voltages
ref_resp=ref_resp(LOAD.Electrodes.m_nodes);
