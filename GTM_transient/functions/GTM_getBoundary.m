function [LOAD,eval_time] = GTM_getBoundary(MESH,PROBLEM,LOAD)
...
% get the time vector
T=0:PROBLEM.TimeSettings.dT:PROBLEM.TimeSettings.nT*PROBLEM.TimeSettings.dT;
% % total time steps nT
...
for i=1:size(PROBLEM.LoadDef.BC,1)
    ...
    if strncmpi(PROBLEM.LoadDef.BC{i,2},'trans',5)
        ...
        LOAD.Boundary.Transfer.Value{N_bt}=bsxfun(@times,LOAD.Boundary.Transfer.Elements{N_bt}(:,3),PROBLEM.LoadDef.BC{i,3}(T).*PROBLEM.LoadDef.BC{i,4});
    ...
    elseif strncmpi(PROBLEM.LoadDef.BC{i,2},'dirich',6)
        ...
        LOAD.Boundary.Dirichlet.Value{N_bd}=bsxfun(@times,ones(length(LOAD.Boundary.Dirichlet.Nodes{N_bd}),1),PROBLEM.LoadDef.BC{i,3}(T));
    elseif strncmpi(PROBLEM.LoadDef.BC{i,2},'flux',4)
        ...
        LOAD.Boundary.Flux.Value{N_bf}=bsxfun(@times,LOAD.Boundary.Flux.Elements{N_bf}(:,3),PROBLEM.LoadDef.BC{i,3}(T));
