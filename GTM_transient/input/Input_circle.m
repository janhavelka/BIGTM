...
PROBLEM.General.MeshName = 'circle_2_inclusions_real';
% mesh file for reconstruction/computation
PROBLEM.General.MeshName_comp = 'circle_2_inclusions_comp';
% Name of output file
PROBLEM.General.SaveName = NameSequence('results_circle_inclusions','output',[],'.mat',[]);
% SaveMode
...
% PROBLEM.Measurements.delete_measurements={[-0.25 0.5;-0.25 -0.5;0.25 -0.5;0.25 0.5];...
%                                        [-0.5 0.25;-0.5 -0.25;0.5 -0.25;0.5 0.25]};
PROBLEM.Measurements.delete_measurements={[-0.5 0.7;-0.5 -0.5;0.6 -0.5]};
...
PROBLEM.LoadDef.BC = {[-0.5 0.7;-0.5 -0.5;0.6 -0.5],'transfer',@(t) 15+0.5.*(cos(2*pi/(PROBLEM.TimeSettings.nT*PROBLEM.TimeSettings.dT).*t*5*ndays))-1.4.*(cos(2*pi/(PROBLEM.TimeSettings.nT*PROBLEM.TimeSettings.dT).*t*ndays))-0.5.*(cos(2*pi/(PROBLEM.TimeSettings.nT*PROBLEM.TimeSettings.dT).*t)),1e1;... inner left boundary
    [-0.5 0.7;0.6 0.7;0.6 -0.5],'transfer',@(t) -5.1+0.4.*sin(2*pi/(PROBLEM.TimeSettings.nT*PROBLEM.TimeSettings.dT).*t*10*ndays)-4.2.*sin(2*pi/(PROBLEM.TimeSettings.nT*PROBLEM.TimeSettings.dT).*t*ndays*0.99)-1.4.*(cos(2*pi/(PROBLEM.TimeSettings.nT*PROBLEM.TimeSettings.dT).*t)),1e1};... outer left boundary
